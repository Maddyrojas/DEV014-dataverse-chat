export default [
  {// 01_Volcán Arenal
    "id": "01_lugarTuristico",
    "name": "Volcán Arenal",
    "location": "Alajuela",
    "shortDescription": "Impresionante volcán activo con vistas panorámicas.",
    "description": "El Volcán Arenal es uno de los destinos turísticos más destacados de Costa Rica. Con su cono perfecto y actividad volcánica ocasional, ofrece experiencias inolvidables para los amantes de la naturaleza y la aventura. Explora los senderos cercanos, relájate en aguas termales en las faldas del Volcán y maravíllate con la biodiversidad que rodea este icónico volcán. Observa una gran variedad de flora y fauna entre ellos aves, reptiles y mamíferos en su hábitat natural, mientras disfrutas la satisfacción de contribuir a la conservación de este ecosistema.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/01_lugarTuristico.jpg",
    "tipoTurismo": "turismo de aventura",
    "gastoPromedio": 150,
    "guideName": "Alexis",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Alexis.jpeg",
    "facts": {
      "museo": "Museo de Volcanología",
      "compras": "Arte Tico",
      "alimentacion": "Restaurante El Cráter"
    }
  },
  {//02_Parque Nacional Manuel Antonio
    "id": "02_lugarTuristico",
    "name": "Parque Nacional Manuel Antonio",
    "location": "Puntarenas",
    "shortDescription": "Reserva natural con playas prístinas y diversidad de vida silvestre.",
    "description": "Manuel Antonio es un paraíso costero que combina impresionantes playas de arena blanca con exuberantes selvas tropicales. Este parque nacional es hogar de una increíble diversidad de vida silvestre, incluyendo monos capuchinos, perezosos, iguanas, pizotes y aves exóticas. Disfruta de emocionantes caminatas por la selva, relájate en las playas prístinas y observa el atardecer en este destino único. El Parque Nacional Manuel Antonio es un lugar perfecto para quienes buscan una experiencia única que combina aventura, amor por la naturaleza y relajación.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/02_lugarTuristico.jpg",
    "tipoTurismo": "turismo de playa",
    "gastoPromedio": 120,
    "guideName":"Tico",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Tico%20el%20Cocodrilo.jpeg",
    "facts": {
      "museo": "undefined",
      "compras": "Mercado Artesanal Manuel Antonio",
      "alimentacion": "Restaurante Mariscos del Pacífico"
    }
  },
  {//03_Reserva Biológica Bosque Nuboso Monteverde
    "id": "03_lugarTuristico",
    "name": "Reserva Biológica Bosque Nuboso Monteverde",
    "location": "Puntarenas",
    "shortDescription": "Explora un bosque nuboso lleno de biodiversidad única.",
    "description": "La Reserva Biológica Bosque Nuboso Monteverde es un tesoro natural que alberga una increíble diversidad de flora y fauna. Observa árboles centenarios, orquídeas exóticas, plantas epífitas y una gran variedad de helechos. Camina por los senderos suspendidos entre las copas de los árboles, observa aves coloridas y mariposas exóticas, y sumérgete en la magia de un bosque nuboso. Disfruta de la adrenalina de caminar por los senderos suspendidos, la belleza de la flora y fauna, y la satisfacción de contribuir a la conservación de este ecosistema invaluable. Una experiencia única para los amantes de la naturaleza y la aventura.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/03_lugarTuristico.jpg",
    "tipoTurismo": "turismo de aventura",
    "gastoPromedio": 130,
    "guideName":"Tico",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Tico%20el%20Cocodrilo.jpeg",
    "facts": {
      "museo": "Museo de Historia Natural Monteverde",
      "compras": "Arte y Artesanía Monteverde",
      "alimentacion": "Café Sostenible Monteverde"
    }
  },
  {//04_Parque Nacional Tortuguero
    "id": "04_lugarTuristico",
    "name": "Parque Nacional Tortuguero",
    "location": "Limón",
    "shortDescription": "Refugio de tortugas marinas rodeado de canales y exuberante vegetación.",
    "description": "Tortuguero es un paraíso natural donde las tortugas marinas anidan en sus playas, observación que te permitira conectarte con la naturaleza y conocer la importancia de la conservación. Explora los canales en bote, camina por las playas, observa la vida silvestre única y participa en la conservación de las tortugas. Observa una gran variedad de aves, reptiles y mamíferos en su hábitat natural. Con su belleza natural y enfoque en la sostenibilidad, Tortuguero ofrece una experiencia única en la costa caribeña de Costa Rica.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/04_lugarTuristico.jpg",
    "tipoTurismo": "turismo de aventura",
    "gastoPromedio": 140,
    "guideName": "Carlos",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Carlos.jpeg",
    "facts": {
      "museo": "Museo de Conservación de Tortugas",
      "compras": "Artesanías del Caribe",
      "alimentacion": "Restaurante Sabores Tropicales"
    }
  },
  {//05_Parque Nacional Corcovado
    "id": "05_lugarTuristico",
    "name": "Parque Nacional Corcovado",
    "location": "Puntarenas",
    "shortDescription": "Reserva natural con selva tropical y vida silvestre diversa.",
    "description": "Corcovado es una joya natural que alberga una de las selvas tropicales más biodiversas del mundo. Descubre una amplia variedad de especies, observa jaguares, monos aulladores, tapires, perezosos y una gran variedad de aves en su hábitat natural. Corcovado es un refugio para la vida silvestre y un paraíso para los amantes de la naturaleza ,perfecto para quienes buscan una experiencia única que combina aventura, naturaleza y aprendizaje. Mientras exploras senderos rodeados de exuberante vegetación admira la belleza de la selva virgen y siente la paz que te ofrece este entorno natural único.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/05_lugarTuristico.jpg",
    "tipoTurismo": "turismo de aventura",
    "gastoPromedio": 160,
    "guideName":"Tico",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Tico%20el%20Cocodrilo.jpeg",
    "facts": {
      "museo": "undefined",
      "compras": "Artesanías Osa Peninsula",
      "alimentacion": "Restaurante Selva Sabrosa"
    }
  },
  {//06_Parque Nacional Rincón de la Vieja
    "id": "06_lugarTuristico",
    "name": "Parque Nacional Rincón de la Vieja",
    "location": "Guanacaste",
    "shortDescription": "Volcán activo con aguas termales y biodiversidad única.",
    "description": "Rincón de la Vieja ofrece una experiencia única con su paisaje volcánico de cráteres inactivos, aguas termales relajantes y biodiversidad sorprendente invitandote a descubrir un mundo de aventura y biodiversidad. Explora senderos que te llevan a cascadas, lagunas y fumarolas. Atrevete a sumérgirte en aguas termales relajantes y curativas, tambien admira la diversidad de aves y monos. Dejate envolver por el calor y la tranquilidad de este oasis natural. Un destino perfecto para los amantes de la aventura y la relajación natural.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/06_lugarTuristico.jpg",
    "tipoTurismo": "turismo de aventura",
    "gastoPromedio": 140,
    "guideName": "Danny",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Daniel.jpeg",
    "facts": {
      "museo": "undefined",
      "compras": "Artesanías Guanacaste",
      "alimentacion": "Restaurante Volcánico"
    }
  },
  {//07_Teatro Nacional de Costa Rica
    "id": "07_lugarTuristico",
    "name": "Teatro Nacional de Costa Rica",
    "location": "San José",
    "shortDescription": "Joyero arquitectónico y cultural en el corazón de San José.",
    "description": "El Teatro Nacional es una joya arquitectónica y cultural que ha sido parte integral de la escena artística de Costa Rica desde su inauguración en 1897 y tambien un punto de encuentro importante. Su elegante diseño neoclásico, magníficos frescos y presentaciones artísticas hacen de este teatro un hito turistico cautivador. Disfruta de conciertos, óperas y eventos culturales majestuosos en este emblemático lugar, siente la magia del arte en su máxima expresión. Perfecto para amantes de la cultura, arte y la vida de la ciudad.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/07_lugarTuristico.jpg",
    "tipoTurismo": "turismo cultural",
    "gastoPromedio": 50,
    "guideName": "Tuki",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Tuki%20el%20quetzal.jpeg",
    "facts": {
      "museo": "Museo del Teatro Nacional",
      "compras": "Tienda de Souvenirs",
      "alimentacion": "Café del Arte"
    }
  },
  {//08_Playa Conchal
    "id": "08_lugarTuristico",
    "name": "Playa Conchal",
    "location": "Guanacaste",
    "shortDescription": "Playa de arena blanca con conchas marinas en la costa del Pacífico.",
    "description": "Playa Conchal es conocida por su distintiva arena de conchas marinas trituradas, creando un paisaje magico y único en la costa del Pacífico. Disfruta de aguas cristalinas, perfectas para nadar y practicar snorkel. Observa una gran variedad de peces tropicales y descubre la belleza del mundo submarino en este entorno paradisíaco. Esta playa ofrece un entorno paradisíaco rodeado de vegetación tropical y es un destino popular para relajarse y disfrutar del sol con la brisa fresca del mar en tu rostro. destino ideal para parejas y amantes de la naturaleza.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/08_lugarTuristico.jpg",
    "tipoTurismo": "turismo de playa",
    "gastoPromedio": 80,
    "guideName": "Danny",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Daniel.jpeg",
    "facts": {
      "museo": "undefined",
      "compras": "Arte y Artesanía Guanacaste",
      "alimentacion": "Restaurante Mariscos Conchal"
    }
  },
  {//09_Parque Nacional Volcán Poás
    "id": "09_lugarTuristico",
    "name": "Parque Nacional Volcán Poás",
    "location": "Alajuela",
    "shortDescription": "Cráter activo con laguna ácida y bosques nubosos.",
    "description": "El Parque Nacional Volcán Poás alberga uno de los cráteres más grandes y activos del mundo, te invita a descubrir un mundo fascinante en las profundidades de la tierra. Asómate al borde del cráter Poás y siente la energía de la naturaleza en su máxima expresión. Observa la laguna ácida de color verde intenso y las fumarolas que emanan del volcán, creando un espectáculo natural único. Explora senderos rodeados de bosques nubosos. Este destino ofrece una experiencia única para los amantes de la geología y la naturaleza. Ten en cuenta las variaciones climáticas al planificar tu visita.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/09_lugarTuristico.jpg",
    "tipoTurismo": "turismo de aventura",
    "gastoPromedio": 70,
    "guideName": "Alexis",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Alexis.jpeg",
    "facts": {
      "museo": "Centro de Interpretación Geológica",
      "compras": "Tienda de Recuerdos Poás",
      "alimentacion": "Café Vista al Cráter"
    }
  },
  {//10_Parque Nacional Cahuita
    "id": "10_lugarTuristico",
    "name": "Parque Nacional Cahuita",
    "location": "Limón",
    "shortDescription": "Reserva marina con arrecifes de coral y selva tropical.",
    "description": "Parque Nacional Cahuita es un paraíso natural que combina la belleza del arrecife de coral más desarrollado del Caribe costarricense con la exuberancia de la selva tropical. Explora el arrecife de coral, hogar de una diversidad única de vida marina, y camina por senderos que atraviesan la selva. Este parque nacional es una joya en la costa caribeña de Costa Rica, ideal para amantes de la naturaleza. Practica snorkel o buceo y vive una experiencia inolvidable en el corazón del Caribe. ",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/10_lugarTuristico.jpg",
    "tipoTurismo": "turismo de aventura",
    "gastoPromedio": 90,
    "guideName": "Carlos",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Carlos.jpeg",
    "facts": {
      "museo": "Museo Marino Cahuita",
      "compras": "Artesanías Caribeñas",
      "alimentacion": "Restaurante Sabores del Mar"
    }
  },
  {//11_Isla Tortuga
    "id": "11_lugarTuristico",
    "name": "Isla Tortuga",
    "location": "Puntarenas",
    "shortDescription": "Isla paradisíaca con aguas cristalinas y zonas acuáticas.",
    "description": "Isla Tortuga es un refugio tropical con playas de arena blanca y aguas cristalinas, explora la vida marina en un entorno paradisíaco y vive la emoción de practicar deportes acuáticos en un escenario único. Relájate bajo el sol, es un destino perfecto para quienes buscan escapar del bullicio y disfrutar de la tranquilidad del mar. Relájate bajo el sol, toma un baño refrescante en las aguas cristalinas y disfruta de la paz y la belleza natural que te rodea. Un destino ideal para escapar y disfrutar de la belleza natural en el Golfo de Nicoya.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/11_lugarTuristico.jpg",
    "tipoTurismo": "turismo de playa",
    "gastoPromedio": 120,
    "guideName":"Tico",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Tico%20el%20Cocodrilo.jpeg",
    "facts": {
      "museo": "undefined",
      "compras": "Tienda de Recuerdos Isla Tortuga",
      "alimentacion": "Restaurante Tropical"
    }
  },
  {//12_Santa Teresa
    "id": "12_lugarTuristico",
    "name": "Santa Teresa",
    "location": "Puntarenas",
    "shortDescription": "Playa tranquila con ambiente bohemio y excelentes olas para surfear.",
    "description": "Santa Teresa es una joya escondida en la Península de Nicoya, conocida por su ambiente bohemio y excelentes condiciones para el surf en olas de clase mundial, disfruta de clases de surf, alquila una tabla y experimenta la emoción de conquistar las olas. Relájate en sus playas de arena blanca, explora la vida nocturna local, prueba la deliciosa gastronomía local y conoce a gente interesante en este ambiente bohemio. Vive la autenticidad de este destino de playa. Perfecto para quienes buscan una experiencia relajada y única junto al mar.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/12_lugarTuristico.jpg",
    "tipoTurismo": "turismo de playa",
    "gastoPromedio": 100,
    "guideName":"Tico",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Tico%20el%20Cocodrilo.jpeg",
    "facts": {
      "museo": "undefined",
      "compras": "Tiendas Locales",
      "alimentacion": "Restaurante Mar y Arena"
    }
  },
  {//13_Reserva Biológica Isla del Caño
    "id": "13_lugarTuristico",
    "name": "Reserva Biológica Isla del Caño",
    "location": "Puntarenas",
    "shortDescription": "Isla biológica con arrecifes de coral y rica biodiversidad marina.",
    "description": "La Reserva Biológica Isla del Caño es un paraíso submarino en el Pacífico para los amantes del buceo y la biodiversidad marina. Explora arrecifes de coral, nada con tiburones y descubre una amplia variedad de peces tropicales. Esta isla es un refugio para la conservación marina y ofrece una experiencia inolvidable. Observa tortugas marinas, delfines, mantarrayas y una gran cantidad de peces de colores que habitan en este ecosistema único. Déjate cautivar por la belleza del mar, la majestuosidad de la vida marina y la paz que te ofrece este refugio natural. Un viaje que te dejará recuerdos imborrables.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/13_lugarTuristico.jpg",
    "tipoTurismo": "turismo de aventura",
    "gastoPromedio": 130,
    "guideName":"Tico",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Tico%20el%20Cocodrilo.jpeg",
    "facts": {
      "museo": "undefined",
      "compras": "Tienda de Buceo",
      "alimentacion": "Restaurante Marina del Caño"
    }
  },
  {//14_Playa Hermosa
    "id": "14_lugarTuristico",
    "name": "Playa Hermosa",
    "location": "Guanacaste",
    "shortDescription": "Extensa playa con aguas tranquilas y atardeceres impresionantes.",
    "description": "Playa Hermosa es un refugio de paz con su extensa playa de arena oscura y aguas tranquilas  ideal para nadar, tomar el sol y practicar deportes acuáticos. Disfruta de puestas de sol impresionantes y relájate en un entorno natural, mientras el sol se oculta en el horizonte del Pacífico, creando un espectáculo inolvidable. Esta playa es perfecta para quienes buscan escapar del bullicio y disfrutar de la serenidad del Pacífico. Playa Hermosa es perfecta para familias, parejas y amantes de la naturaleza en general. Disfruta de actividades como surf, natación, snorkel, kayak o simplemente relájate en la arena.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/14_lugarTuristico.jpg",
    "tipoTurismo": "turismo de playa",
    "gastoPromedio": 80,
    "guideName": "Danny",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Daniel.jpeg",
    "facts": {
      "museo": "undefined",
      "compras": "Tienda de Surf",
      "alimentacion": "Restaurante Mar y Cielo"
    }
  },
  {//15_Parque Nacional Barra Honda
    "id": "15_lugarTuristico",
    "name": "Parque Nacional Barra Honda",
    "location": "Guanacaste",
    "shortDescription": "Sistema de cuevas subterráneas y formaciones rocosas únicas.",
    "description": "Barra Honda es famoso por su sistema de cuevas subterráneas y formaciones rocosas únicas conocido como un viaje al centro de la tierra. Desciende a las profundidades de la tierra para explorar estas cuevas profundas y admira estalactitas y estalagmitas impresionantes y siente la adrenalina de recorrer este laberinto natural. Este parque nacional es una joya geológica y un destino fascinante para los amantes de la espeleología. bserva fósiles marinos, descubre la historia geológica de la región y aprende sobre la importancia de la conservación de este ecosistema único.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/15_lugarTuristico.jpg",
    "tipoTurismo": "turismo de aventura",
    "gastoPromedio": 60,
    "guideName": "Danny",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Daniel.jpeg",
    "facts": {
      "museo": "Centro de Interpretación Geológica",
      "compras": "Arte y Artesanía Guanacaste",
      "alimentacion": "Café Roca Subterránea"
    }
  },
  {//16_Parque Nacional Carara
    "id": "16_lugarTuristico",
    "name": "Parque Nacional Carara",
    "location": "Puntarenas",
    "shortDescription": "Refugio de aves con senderos naturales y densa vegetación.",
    "description": "Carara es un paraíso para observadores de aves con su densa vegetación y una gran variedad de especies. Observa monos traviesos, perezosos en las copas de los árboles y una gran variedad de reptiles y anfibios en este ecosistema único en su hábitat natural. Este parque nacional es perfecto para amantes de la naturaleza y la fotografía. Para los amantes de la aventura, el parque ofrece la posibilidad de realizar actividades como senderismo, ciclismo de montaña y kayak. Recorre senderos que te llevarán a descubrir ríos cristalinos, miradores con vistas impresionantes y la belleza incomparable del bosque tropical.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/16_lugarTuristico.jpg",
    "tipoTurismo": "turismo de aventura",
    "gastoPromedio": 70,
    "guideName":"Tico",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Tico%20el%20Cocodrilo.jpeg",
    "facts": {
      "museo": "undefined",
      "compras": "Tienda de Souvenirs Carara",
      "alimentacion": "Café Selva Viva"
    }
  },
  {//17_Reserva Nacional Absoluta Cabo Blanco
    "id": "17_lugarTuristico",
    "name": "Reserva Nacional Absoluta Cabo Blanco",
    "location": "Puntarenas",
    "shortDescription": "Reserva natural con playas prístinas y bosques intocados.",
    "description": "Cabo Blanco es una reserva natural que ofrece playas prístinas, bosques intocados y una rica biodiversidad. Recorre playas vírgenes de arena blanca, explora bosques tropicales prístinos y observa una gran variedad de flora y fauna en este oasis de paz y tranquilidad. Explora senderos que te llevan a la costa y admira la belleza natural en su forma más pura. Este destino es perfecto para quienes buscan la tranquilidad de la naturaleza en su estado más salvaje, la reserva ofrece la posibilidad de realizar actividades como senderismo, ciclismo de montaña y observación de aves.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/17_lugarTuristico.jpg",
    "tipoTurismo": "turismo de aventura",
    "gastoPromedio": 100,
    "guideName":"Tico",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Tico%20el%20Cocodrilo.jpeg",
    "facts": {
      "museo": "undefined",
      "compras": "Arte y Artesanía Cabo Blanco",
      "alimentacion": "Restaurante Costa Salvaje"
    }
  },
  {//18_Parque Nacional Chirripó
    "id": "18_lugarTuristico",
    "name": "Parque Nacional Chirripó",
    "location": "San José",
    "shortDescription": "Montaña más alta de Costa Rica con senderos desafiantes y vistas impresionantes.",
    "description": "Chirripó es el hogar de la montaña más alta de Costa Rica y ofrece senderos desafiantes y vistas impresionantes. Supera el desafío de ascender a la cima y sé recompensado con paisajes de ensueño que te llevarán a través de bosques nubosos, páramos y glaciares hasta alcanzar la cima del Cerro Chirripó. Este parque nacional es un destino imperdible para excursionistas y amantes de la naturaleza. Desafía tus límites, disfruta de la paz del bosque nuboso y contempla la belleza incomparable de los paisajes costarricenses. Un viaje que te dejará recuerdos imborrables.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/18_lugarTuristico.jpg",
    "tipoTurismo": "turismo de aventura",
    "gastoPromedio": 120,
    "guideName": "Tuki",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Tuki%20el%20quetzal.jpeg",
    "facts": {
      "museo": "undefined",
      "compras": "Arte y Artesanía Chirripó",
      "alimentacion": "Café Cima de la Montaña"
    }
  },
  {//19_Reserva Biológica Hitoy-Cerere
    "id": "19_lugarTuristico",
    "name": "Reserva Biológica Hitoy-Cerere",
    "location": "Limón",
    "shortDescription": "Humedales y bosques tropicales con una rica variedad de vida silvestre.",
    "description": "Hitoy-Cerere es una reserva biológica que protege humedales y bosques tropicales, creando un hábitat ideal para una variada fauna aves multicolores, ranas exóticas y una gran variedad de reptiles que habitan en este refugio natural. Explora senderos naturales, observa aves exóticas y disfruta de la tranquilidad de este entorno natural. Un destino perfecto para amantes de la biodiversidad y la observación de aves.Hitoy-Cerere tambien ofrece la oportunidad de realizar actividades como senderismo y kayak. Déjate llevar por la adrenalina de explorar este paraíso natural. Una experiencia inolvidable para los amantes de la aventura.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/19_lugarTuristico.jpg",
    "tipoTurismo": "turismo de aventura",
    "gastoPromedio": 80,
    "guideName": "Carlos",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Carlos.jpeg",
    "facts": {
      "museo": "undefined",
      "compras": "Artesanías del Caribe",
      "alimentacion": "Restaurante Selva Encantada"
    }
  },
  {//20_Parque Nacional Tapantí
    "id": "20_lugarTuristico",
    "name": "Parque Nacional Tapantí",
    "location": "Cartago",
    "shortDescription": "Bosques nubosos y cascadas en el corazón de la cordillera de Talamanca.",
    "description": "Tapantí es un tesoro natural con bosques nubosos, cascadas impresionantes y una rica diversidad de flora y fauna en el corazón de Costa Rica. Explora senderos que te sumergen en la belleza de la cordillera de Talamanca. Este parque nacional es perfecto para quienes buscan la conexión con la naturaleza en su estado más puro. Déjate envolver por la neblina y la frescura del bosque nuboso de Tapantí. Observa orquídeas multicolores, bromelias exóticas y una gran variedad de aves que habitan en este ecosistema único.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/20_lugarTuristico.jpg",
    "tipoTurismo": "turismo de aventura",
    "gastoPromedio": 90,
    "guideName": "Brie",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Brie%20la%20colibrie.jpeg",
    "facts": {
      "museo": "undefined",
      "compras": "Arte y Artesanía Cartago",
      "alimentacion": "Café Vista a la Cascada"
    }
  },
  {//21_Parque Nacional Volcán Tenorio
    "id": "21_lugarTuristico",
    "name": "Parque Nacional Volcán Tenorio",
    "location": "Guanacaste",
    "shortDescription": "Cráter activo, aguas celestiales y la famosa catarata Río Celeste.",
    "description": "Tenorio es un destino mágico con su cráter activo, que te invita a vivir una experiencia única en un escenario de belleza natural incomparable. Camina por senderos que te llevan a paisajes surrealistas y sumérgete en pozas de aguas azules. Observa fumarolas que emanan del cráter activo, contempla la impresionante caída de la catarata Río Celeste, un espectáculo natural que te dejará sin aliento. Observa aves multicolores, monos traviesos y una gran variedad de plantas en este ecosistema único. Este parque nacional es un sueño para los amantes de la fotografía y la belleza natural.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/21_lugarTuristico.jpg",
    "tipoTurismo": "turismo de aventura",
    "gastoPromedio": 110,
    "guideName": "Danny",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Daniel.jpeg",
    "facts": {
      "museo": "undefined",
      "compras": "Artesanías Guanacaste",
      "alimentacion": "Restaurante Celestial"
    }
  },
  {//22_Parque Nacional Isla del Coco
    "id": "22_lugarTuristico",
    "name": "Parque Nacional Isla del Coco",
    "location": "Puntarenas",
    "shortDescription": "Reserva marina remota con arrecifes de coral y vida marina única.",
    "description": "Isla del Coco es un paraíso remoto que alberga arrecifes de coral, tiburones martillo y una asombrosa variedad de vida marina. Este parque nacional, declarado Patrimonio de la Humanidad por la UNESCO, ofrece oportunidades únicas para bucear y explorar la riqueza submarina del Pacífico. Un destino imperdible para amantes del buceo y la conservación marina. Su ecosistema alberga gran variedad de especioes endémicas, al visitarlo contribuyes a la protección de este tesoro nacional para que las futuras generaciones puedan visitar y vivir la oportunidad de vivir una aventura submarina incomparable",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/22_lugarTuristico.jpg",
    "tipoTurismo": "turismo de aventura",
    "gastoPromedio": 150,
    "guideName":"Tico",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Tico%20el%20Cocodrilo.jpeg",
    "facts": {
      "museo": "undefined",
      "compras": "Tienda de Buceo",
      "alimentacion": "Restaurante Océano Azul"
    }
  },
  {//23_Playa Tamarindo
    "id": "23_lugarTuristico",
    "name": "Playa Tamarindo",
    "location": "Guanacaste",
    "shortDescription": "Famosa playa con aguas cristalinas y vibrante vida nocturna.",
    "description": "Tamarindo es un icónico destino de playa conocido por sus aguas cristalinas, olas ideales para el surf y su animada vida nocturna, esta ubicada en la inolvidable costa vacacional de Guanacaste. Disfruta de días soleados en la playa, practica deportes acuáticos emocionantes y sumérgete en la atmósfera relajada y divertida de Tamarindo. Con una variedad de restaurantes y bares, este destino tiene algo para todos los gustos.Al caer la noche, Tamarindo se transforma en un escenario de fiesta y diversión. Disfruta de una amplia variedad de discotecas con Ritmos latinos, música en vivo y experiencias inolvidables.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/23_lugarTuristico.jpg",
    "tipoTurismo": "turismo de playa",
    "gastoPromedio": 90,
    "guideName": "Danny",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Daniel.jpeg",
    "facts": {
      "museo": "undefined",
      "compras": "Tiendas Locales Tamarindo",
      "alimentacion": "Restaurante Mar y Fiesta"
    }
  },
  {//24_Parque Nacional Piedras Blancas
    "id": "24_lugarTuristico",
    "name": "Parque Nacional Piedras Blancas",
    "location": "Puntarenas",
    "shortDescription": "Selva virgen y cascadas impresionantes en la Península de Osa.",
    "description": "Piedras Blancas es un tesoro natural en la Península de Osa, con selva virgen, ríos cristalinos y cascadas impresionantes. Explora senderos que te llevan a la riqueza biológica de la región, hogar de jaguares sigilosos, monos traviesos y aves multicolores que te sorprenderán con sus cantos melodiosos. Déjate llevar por la magia de la naturaleza y disfruta de un encuentro inolvidable con la fauna y flora de Costa Rica.Este parque nacional ofrece una experiencia inmersiva en la naturaleza virgen de Costa Rica. Déjate envolver por el sonido del agua cristalina, la brisa fresca y el canto de las aves.",
    "imageUrl": "https://raw.githubusercontent.com/Maddyrojas/DEV014-Dataverse/main/Imagenes%20Dataverse/24_lugarTuristico.jpg",
    "tipoTurismo": "turismo de aventura",
    "gastoPromedio": 100,
    "guideName":"Tico",
    "guideImg": "https://raw.githubusercontent.com/PemRug/DEV014-dataverse-chat/view/imgs/Tico%20el%20Cocodrilo.jpeg",
    "facts": {
      "museo": "undefined",
      "compras": "Artesanías Osa Peninsula",
      "alimentacion": "Restaurante Selva Fresca"
    }
  },
];
